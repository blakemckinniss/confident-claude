...
def redact_text(text: str) -> tuple[str, list[str]]:
    ...
    for name, pattern, replacer in PATTERNS:
        ...
        if name == "env_secret" and not config.safety.redact_env_vars:
            continue
        if name in ("api_key", "bearer_token", "llm_key", "groq_key", "github_token", "aws_key") and not config.safety.redact_api_keys:
            continue
...
def redact_dict(data: dict, depth: int = 0, max_depth: int = 10) -> dict:
    ...
    for key, value in data.items():
        ...
        if any(s in key_lower for s in ("secret", "password", "token", "key", "credential", "auth", "api_key")):
