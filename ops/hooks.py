#!/usr/bin/env python3
"""
Hook System Tool - Unified audit and testing for Claude Code hooks.

Validates hooks against official spec, runs execution tests, and auto-fixes issues.

Usage:
    python3 .claude/ops/hooks.py              # Full audit
    python3 .claude/ops/hooks.py --test       # Run execution tests
    python3 .claude/ops/hooks.py --fix        # Auto-fix common issues
    python3 .claude/ops/hooks.py --prune      # Archive orphaned hooks
...
class HookAuditor:
        ...
        for event_type, event_hooks in hooks.items():
            for hook_config in event_hooks:
                for hook in hook_config.get("hooks", []):
                    ...
                    # Match both formats:
                    # 1. .claude/hooks/script.py (direct path)
                    # 2. cd ... && python script.py (cd + python)
                    match = re.search(r'\.claude/hooks/([\w.-]+\.py)', cmd)
                    if match:
                        registered.add(match.group(1))
                    else:
                        # Match: python script.py at end of command
                        match = re.search(r'python3?\s+([\w.-]+\.py)(?:\s|$)', cmd)
                        if match:
                            registered.add(match.group(1))
        return registered
    ...
    def check_hook_output_format(self, hook_file: str) -> list:
        ...
        # Check if hook imports output_hook_result from synapse_core (which handles spec correctly)
        uses_synapse_output = (
            "from synapse_core import" in content and
            "output_hook_result" in content
        )
        ...
        if event_type == "PreToolUse":
            # Must use hookSpecificOutput wrapper (or import from synapse_core)
            if "permissionDecision" in content or "allow" in content.lower() or "deny" in content.lower():
                if "hookSpecificOutput" not in content and not uses_synapse_output:
    ...
    def check_prompt_type_hooks(self, settings: dict) -> list:
        ...
        Per official docs, they work with any hook event but are most useful for:
        - Stop, SubagentStop, UserPromptSubmit, PreToolUse, PermissionRequest
        """
        hooks = settings.get("hooks", {})
        issues = []

        # Events where prompt-type hooks are most useful per official docs
        recommended_prompt_events = {"Stop", "SubagentStop", "UserPromptSubmit",
            ...
            for hook_config in event_hooks:
                for hook in hook_config.get("hooks", []):
                    if hook.get("type") == "prompt":
                        # Only warn for events not in the recommended list
                        if event_type not in recommended_prompt_events:
                            issues.append((event_type, "prompt_type_unusual_event"))
                            self.add_issue(
                                "WARNING", "SPEC_VIOLATION",
                                f"Prompt-based hook in {event_type} - consider using for Stop/SubagentStop/UserPromptSubmit/PreToolUse/PermissionRequest",
                                spec_ref="Prompt hooks work with any event but most useful for decision events"
                            )
    ...
    def check_orphaned_hooks(self) -> list:
        ...
        ignored_patterns = ['_backup', '_v1_backup', 'test_hooks', '_lib_path']
    ...
    def fix_bare_except(self, hook_file: str) -> bool:
        ...
        if content != original:
            ...
            if self.dry_run:
                self.fixes_applied.append((hook_file, "[DRY-RUN] Would fix bare except Exception: clauses"))
                return True
            ...
            try:
                ...
                self.fixes_applied.append((hook_file, "Fixed bare except Exception: clauses (backup: .bak)"))
                return True
            except OSError as e:
...
MOCK_EVENTS = {
    # SessionStart/PreCompact/Stop/SubagentStop: NO cwd per official spec
    "SessionStart": {"hook_event_name": "SessionStart", "session_id": "test",
                     "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                     "source": "startup"},
    "SessionEnd": {"hook_event_name": "SessionEnd", "session_id": "test", "cwd": "/test",
                   "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                   "reason": "other"},
    "UserPromptSubmit": {"hook_event_name": "UserPromptSubmit", "session_id": "test", "cwd": "/test",
                         "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                         "prompt": "test prompt"},
    "PostToolUse": {"hook_event_name": "PostToolUse", "session_id": "test", "cwd": "/test",
                    "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                    "tool_name": "Read", "tool_input": {"file_path": "/test"},
                    "tool_response": {"filePath": "/test", "success": True}, "tool_use_id": "toolu_test"},
    "PreToolUse": {"hook_event_name": "PreToolUse", "session_id": "test", "cwd": "/test",
                   "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                   "tool_name": "Write", "tool_input": {"file_path": "/test", "content": "test"},
                   "tool_use_id": "toolu_test"},
    "PermissionRequest": {"hook_event_name": "PermissionRequest", "session_id": "test", "cwd": "/test",
                          "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                          "tool_name": "Bash", "tool_input": {"command": "rm -rf /"}},
    "Stop": {"hook_event_name": "Stop", "session_id": "test", "permission_mode": "default",
             "transcript_path": "/test/transcript.jsonl", "stop_hook_active": False},
    "SubagentStop": {"hook_event_name": "SubagentStop", "session_id": "test", "permission_mode": "default",
                     "transcript_path": "/test/transcript.jsonl", "stop_hook_active": False},
    "PreCompact": {"hook_event_name": "PreCompact", "session_id": "test", "permission_mode": "default",
                   "transcript_path": "/test/transcript.jsonl", "trigger": "manual",
                   "custom_instructions": ""},
    "Notification": {"hook_event_name": "Notification", "session_id": "test", "cwd": "/test",
                     "transcript_path": "/test/transcript.jsonl", "permission_mode": "default",
                     "message": "Test notification", "notification_type": "permission_prompt"},
}


def test_hook_execution(hook_path: Path, event_type: str, timeout_ms: int = 500) -> dict:
    """Run hook with mock event and measure execution."""
    event_data = MOCK_EVENTS.get(event_type, MOCK_EVENTS["UserPromptSubmit"])
    result = {"success": False, "duration_ms": 0, "error": None, "stdout": "", "stderr": ""}
    ...
    try:
        ...
        result["stdout"] = process.stdout[:500]
        result["stderr"] = process.stderr[:500]
        result["success"] = process.returncode == 0
        if not result["success"]:
            result["error"] = f"Exit {process.returncode}: {process.stderr[:100]}"
...
def run_execution_tests(settings: dict, threshold_ms: int = 500) -> dict:
    """Run execution tests on all registered hooks."""
    results = {"passed": 0, "failed": 0, "slow": [], "failures": []}
    hooks = settings.get("hooks", {})
        ...
        for hook_config in event_hooks:
            for hook in hook_config.get("hooks", []):
                ...
                if exec_result["success"]:
                    results["passed"] += 1
                    if exec_result["duration_ms"] > threshold_ms:
                        results["slow"].append({"file": hook_file, "ms": exec_result["duration_ms"]})
                else:
                    results["failed"] += 1
                    results["failures"].append({"file": hook_file, "error": exec_result["error"]})
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified hook audit and testing tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="Official Spec: https://docs.anthropic.com/en/hooks-reference"
    )
    parser.add_argument("--test", action="store_true", help="Run execution tests with mock events")
    parser.add_argument("--fix", action="store_true", help="Auto-fix common issues")
    parser.add_argument("--dry-run", action="store_true", help="Preview fixes without applying")
    parser.add_argument("--json", action="store_true", help="Output as JSON")
    parser.add_argument("--strict", action="store_true", help="Treat warnings as errors")
    parser.add_argument("--prune", action="store_true", help="Archive orphaned hooks")

    args = parser.parse_args()

    # Load settings for both modes
    if not SETTINGS_FILE.exists():
        print(f"❌ Settings not found: {SETTINGS_FILE}", file=sys.stderr)
        sys.exit(1)
    ...
    if args.test:
        ...
        test_results = run_execution_tests(settings)
